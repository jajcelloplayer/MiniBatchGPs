...
foreach(simSet = 1:n_sets) %dopar% {
  ...
  beta <- matrix(as.numeric(start_vals[1, 1:4]), ncol = 1)
  sigma2 <- start_vals[1, 5]
  alpha <- alpha_vals[which.min(abs(start_vals[1, 6] - alpha_vals))]
  omega <- omega_vals[which.min(abs(start_vals[1, 7] - omega_vals))]
  ...
  save(
    file = paste0("../LargeResults/BG_Discrete_Large_", simSet, ".RData"),
