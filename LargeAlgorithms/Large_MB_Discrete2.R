####
## MiniBatch Discrete (Large) Sets 26-50
####
...
foreach(simSet = 25+(1:n_sets)) %dopar% {
  for(M in c(2^(1:6))){
    ...
    alpha <- alpha_vals[which.min(abs(start_vals[simSet, 6] - alpha_vals))]
    omega <- omega_vals[which.min(abs(start_vals[simSet, 7] - omega_vals))]
    ...
    save(
      file = paste0("../LargeResults/MB_Discrete_Large_", simSet, "_M_", M, ".RData"),
