...
foreach(simSet = 1:n_sets) %dopar% {
  for(M in c(2^(1:6))){
    ...
    kfold_time <- system.time({
      ...
      for(i in 1:tot_epochs){
        ...
        for(l in 1:M){
          ...
          ## MH test
          pri <- sum(dnorm(prop[1:2], mean = m2, sd = std, log = TRUE)) - 
            sum(dnorm(
          ...
          MH <- (Lambda_star + pri - log(runif(1))) > 0
          ...
          ## AMCMC Update
          amcmc <- AMCMC.update(matrix(c(real_trans(omega, omega_min, omega_max), 
                                         real_trans(alpha, alpha_min, alpha_max)), ncol=1), 
                                amcmc$mn, amcmc$var,
                                d)
    ...
    save(
      file = paste0("../Results/MB_Cont_Set_", simSet, "_M_", M, ".RData"),
